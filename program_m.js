...
window.onload = function () {
	...
	$.get("https://lab.weathermap.co.jp/MSMGPV_point/get_latest.php", function (timestr){
...
function selectAmedas(region, pref, amedas) {
    ...
    $.when(
        ...
        points = { ...msmgpvData[0] };
	Object.keys(msmgpvExData[0]).forEach(function (key) {
	    if (!points.hasOwnProperty(key)) {
		points[key] = msmgpvExData[0][key];
	    }
	});
        $('#selectPoint').children().remove();
        Object.keys(points).forEach(function (a) {
	...
	for (var ft = 0; ft <= ftmax; ft += 3) {
		...
		levs.forEach(function (lev) {
			...
			var thresholdColors = {
			    ...
			    300: [-63, -60, -57, -54, -51, -48, -45, -42, -39, -36, -33, -30, 300],
                        };
                        var tempcolors = ['#a0a0a0', '#ffffff', '#00ffff', '#00b0ff', '#0070ff', '#008000', '#00c000', '#00ff00', '#ffff00', '#ffc000', '#ff8000', '#ff0000', '#ff00ff', '#800080'];
